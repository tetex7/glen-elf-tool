...
#ifndef _elf_packed
#	define _elf_packed  __attribute__((packed))
#endif

/* The ELF file header.  This appears at the start of every ELF file.  */

#define EI_NIDENT (16)
#define EI_EXT_NIDENT (7)
#define EI_K_EXT_STR (EI_EXT_NIDENT-3)

#define K_LOADER_SET_BIT_FLAG(a) (1 << (a))

#define K_LOADER_NO_LOADER_FEATURES 0
#define K_LOADER_MK_VMS   K_LOADER_SET_BIT_FLAG(0)
#define K_LOADER_STANDARD_FEATURE_SET (K_LOADER_MK_VMS)

#define K_KEN_BIN_EXT_STR "KREN"
#define K_KEN_DRIVER_EXT_STR "KDIV"
#define K_KEN_MODULE_EXT_STR "KMOD"

typedef union _elf_packed
{
	struct _elf_packed
	{
		Elf64_Half k_vid;
		Elf64_Byte k_requested_loader_feature_flags;
		Elf64_Char k_ext_str[EI_K_EXT_STR]; // A 4 byte Known length string
	} ext_fields;
	Elf64_Byte ext_ident[EI_EXT_NIDENT];
} Elf64_Ext_Ident;

#if __has_builtin(_Static_assert)
_Static_assert(sizeof(Elf64_Ext_Ident) == EI_EXT_NIDENT, "Elf_Ext_Ident size mismatch");
#else
typedef Elf64_Byte ___7_c_elf_static_assert[(sizeof(Elf64_Ext_Ident) == EI_EXT_NIDENT) ? 1 : -1];
#endif
...
typedef union _elf_packed
{
	struct _elf_packed
	{
		Elf64_Byte magic[4];     // 0x7F, 'E', 'L', 'F'
		Elf64_Byte eclass;       // 1 = 32-bit, 2 = 64-bit
		Elf64_Byte data;         // 1 = little endian, 2 = big endian
		Elf64_Byte version;      // ELF version (should be 1)
		Elf64_Byte osabi;        // OS/ABI identification
		Elf64_Byte abiversion;   // ABI version
		Elf64_Ext_Ident ext_elf_k_data;       // Extended data for the kernel ELF
	} fields;
	Elf64_Byte ident[EI_NIDENT];
} Elf64_Ident;

#if __has_builtin(_Static_assert)
_Static_assert(sizeof(Elf64_Ident) == EI_NIDENT, "Elf_Ident size mismatch");
#else
typedef Elf64_Byte ___16_c_elf_static_assert[(sizeof(Elf64_Ident) == EI_NIDENT) ? 1 : -1];
#endif
...
typedef struct
{
  Elf64_Ident	e_ident;        /* Magic number and other info */
