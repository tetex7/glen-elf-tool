...
if [[ "$detected_os" == "linux" ]]; then
    ...
    readonly DEFAULT_TOOLS=(python3 clang cmake ctest ninja git)
else
    # The tools that should be on an generic system
    readonly DEFAULT_TOOLS=(gcc cmake ctest ninja git)
fi
...
function clean()
{
    ...
    rm -fv ./trt_loader-*.tar.gz
    rm -frv ./_deps
    rm -frv ./pico-sdk
    rm -frv ./generated
    rm -frv ./pioasm
    rm -frv ./pioasm-install
